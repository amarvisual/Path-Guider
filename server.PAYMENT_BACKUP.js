...
Be specific about what you observe — mention actual things you see like "the right-leaning slant suggests...", "the heavy pressure indicates...". Make it feel personal and insightful.`;
  ...
  console.error('Handwriting AI error:', lastErr?.message);
...
Be specific about what you actually see: size, slant, pressure, legibility, underlines, loops, flourishes. Make it feel like a premium personalized reading.`;
  ...
  for (const key of API_KEYS) {
    try {
      ...
      const analysis = {
        summary: `AI Graphology Reading for ${name} by Path Guider`,
        personality: parse('PERSONALITY'),
      ...
      return res.json({ success: true, analysis, name, email });
    } catch (err) {
      lastErr = err;
      if (!err.message?.includes('quota') && !err.message?.includes('429')) break;
    }
  }
  console.error('Signature AI error:', lastErr?.message);
  res.status(500).json({ error: 'AI analysis failed. Please try again.' });
});
...
// Step 2B: Free Delivery (For 100% Discount Codes like MYKRISHNA)
app.post('/signature/deliver-free', async (req, res) => {
  const { name, email, traits, promoCode } = req.body;
  
  if (!promoCode || !PROMO_CODES[promoCode.toUpperCase()] || PROMO_CODES[promoCode.toUpperCase()].discountPercentage !== 100) {
    return res.status(400).json({ error: 'Invalid free promo code' });
  }

  const analysis = analyzeSignature(traits);

  // Send signature email in background
  if (email) {
    (async () => {
      try {
